...
$( document ).ready(function(){
    ...
    if(returnBrowserEngine() == "webkit") { // add webkit/blink only functions here
        $('head').append('<link rel="stylesheet" type="text/css" href="{{ '/assets/css/webkit.css' | relative_url }}">');
    }
    ...
    $( "#win_container" ).on('mousedown','.win_titb_min', function() {
        $( this ).attr("src", "{{ '/assets/img/min_dep.png' | relative_url }}");
        $( this ).mouseup(function() {
            $( this ).attr("src", "{{ '/assets/img/min.png' | relative_url }}");
    ...
    $( "#win_container" ).on('mousedown','.win_titb_max', function() {
        $( this ).attr("src", "{{ '/assets/img/max_dep.png' | relative_url }}");
        $( this ).mouseup(function() {
            $( this ).attr("src", "{{ '/assets/img/max.png' | relative_url }}");
            var windc_id = $( this ).parent().attr("id");
            var windc_id_splt = windc_id.split("_");
            var win_id = windc_id_splt[2];
            var window_id = "#window_" + win_id;
    ...
    $( "#win_container" ).on('mousedown','.win_titb_close', function() {
        $( this ).attr("src", "{{ '/assets/img/close_dep.png' | relative_url }}");
        $( this ).mouseup(function() {
            $( this ).attr("src", "{{ '/assets/img/close.png' | relative_url }}");
            var windc_id = $( this ).parent().attr("id");
            //alert(windc_id);
            var windc_id_splt = windc_id.split("_");
            var win_id = windc_id_splt[2];
...
function makeWindow(icon, title, frameurl, taskbar, res, wid, hei) { /* IT WORKS!!! What's the catch? */
    ...
    if(res != false && taskbar != false) { // all buttons
        var sec7 = '"><img src="{{ '/assets/img/min.png' | relative_url }}" class="win_titb_min"><img src="{{ '/assets/img/max.png' | relative_url }}" class="win_titb_max"><img src="{{ '/assets/img/close.png' | relative_url }}" class="win_titb_close"></div></div></div>';
    } else if(res == false && taskbar == false) { // only close
        var sec7 = '"><img src="{{ '/assets/img/close.png' | relative_url }}" class="win_titb_close"></div></div></div>';
    } else if (res == false) { // max. shaded out
        var sec7 = '"><img src="{{ '/assets/img/min.png' | relative_url }}" class="win_titb_min"><img src="{{ '/assets/img/max_disabled.png' | relative_url }}" class="win_titb_max_dis"><img src="{{ '/assets/img/close.png' | relative_url }}" class="win_titb_close"></div></div></div>';
    } else if (taskbar == false) { // no minimize button
        var sec7 = '"><img src="{{ '/assets/img/max.png' | relative_url }}" class="win_titb_max"><img src="{{ '/assets/img/close.png' | relative_url }}" class="win_titb_close"></div></div></div>';
    }
...
function addTaskbar(win_id, icon, title) {
    ...
    if(icon != false) {
        ...
        // icon not present
        var placeholder = "{{ '/assets/img/placeholder.png' | relative_url }}";
        var sec1 = '<button type="button" class="win_tb_button" id="win_tb_win_';
