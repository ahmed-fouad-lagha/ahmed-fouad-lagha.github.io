...
function makeWidget(icon, title, html, taskbar, min, ques, center, wid, hei, cusClass) { /* IT WORKS!!! What's the catch? */
    ...
    if(min == true) {
        // minimize shown
        var sec7 = '"><img src="{{ '/assets/img/min.png' | relative_url }}" class="win_titb_min" id"_"><img src="{{ '/assets/img/max_disabled.png' | relative_url }}" class="win_titb_max_dis" id"_"><img src="{{ '/assets/img/close.png' | relative_url }}" class="win_titb_close" id"_"></div></div>';
    } else {
...
function windowsError(icon, title, msg) {
    if (icon == 1) {
        var iconURL = "{{ '/assets/img/error.png' | relative_url }}";
    }
